...
class Request(request.Request):
    ...
    def request_id(self) -> str | None:
        ...
        This property first checks ``req.context.request_id`` (which is set by
        :class:`falcon.RequestIDMiddleware` when it generates or extracts a request ID),
        and falls back to reading the ``X-Request-ID`` header directly if the context
        attribute is not set.

        This ensures that the request ID is accessible regardless of whether it was:

        * Provided by the client in the ``X-Request-ID`` header
        * Generated by :class:`falcon.RequestIDMiddleware`

        Returns ``None`` if neither the context attribute nor the header is present,
        or if the value is an empty string.

        This header is commonly used for request tracing and correlation across
        services.

        .. versionadded:: 4.1
        """
        # First check if middleware set the request_id in context
        context_id = getattr(self.context, 'request_id', None)
        if context_id:
            return context_id
        # Fall back to reading the header directly
        # Return None for empty strings to be consistent with middleware behavior
        return self.get_header('X-Request-ID') or None
